...
public class PhaoHoa {
    ...
    public PhaoHoa() {
        ...
        Point2D.Double hatBan_vantoc = new Point2D.Double(0, -12);
        int hatBan_thoigiansong = Helper.GetRandomInt(30, 50);

        hatBan = new HatBan(hatBan_toado, hatBan_vantoc, hatBan_thoigiansong, mauSac);
    }


    // khi phát nổ, khởi tạo danh sách các hạt sáng ngẫu nhiên
    void PhatNo() {
        int n = Helper.GetRandomInt(60, 100);
        ...
        for (int i = 0; i < n; i++) {
            ...
            Color mausac = new Color(mauSac.getRGB());

            Hat hat = new Hat(hatBan.ToaDo, vantoc, thoigiansong, mausac);
            listHat.add(hat);
        }
    }
